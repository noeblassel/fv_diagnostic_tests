...
- `chunk_batches` : gradient steps per training chunk (default 5)
- `test_batches`  : batches used for each validation call (default 15)
- `T_final`       : total chunk budget per config used for GP extrapolation
...
function freeze_thaw_bo_search(build_run, lb, ub;
    ...
    # Run one training chunk and return the test loss
    function eval_chunk!(run)
        run_epoch!(run, chunk_batches, false)
    ...
    println("  [FTBO] Initialising ($n_init random configs, $chunk_batches batches/chunk)...")
    for i in 1:n_init
...
const INPUT_DIM_DS   = 50
const BETA_LIMS      = (1.0, 3.0)
const CHUNK_BATCHES  = 5      # batches per chunk  (6 chunks × 5 = 30 total, same as tune_bo.jl)
const TEST_BATCHES   = 15
const T_FINAL        = 6      # GP extrapolation horizon (chunks)
...
const RETRAIN_BATCHES = CHUNK_BATCHES * T_FINAL   # 30 — matches tune_bo.jl
...
JLD2.jldsave("best_ftbo_cnn.jld2", model_state = Flux.state(best_run_cnn.model))
println("Saved → best_ftbo_cnn.jld2")
...
JLD2.jldsave("best_ftbo_ds.jld2", model_state = Flux.state(best_run_ds.model))
println("Saved → best_ftbo_ds.jld2")

# ── Overall winner ────────────────────────────────────────────────────────────
if loss_cnn <= loss_ds
    println("\nOverall winner: CNN (loss=$(loss_cnn))")
    JLD2.jldsave("best_ftbo.jld2", model_state = Flux.state(best_run_cnn.model))
else
    println("\nOverall winner: DeepSet (loss=$(loss_ds))")
    JLD2.jldsave("best_ftbo.jld2", model_state = Flux.state(best_run_ds.model))
end
println("Saved → best_ftbo.jld2")
