using Flux, JLD2, MLUtils, Printf
...
rseed = 2027
rng = Xoshiro(rseed)
model_state = JLD2.load("./history/epoch_51_2025.jld2", "model_state")

ecdf_resolution = 64
βlims = (1.0,1.0) # temperature range
model = RNNDiagnostic(input_dim=ecdf_resolution, rng=rng)
...
function eval_metrics(model;
    ...
    @showprogress for n = 1:num_samples
        ...
        for j = 1:ntraj
            ...
            for k = 1:traj_length
                time_bin = get_bin(k / decorr_steps, 0.0, 2.0, nbins_time)
                for (i, th) in enumerate(thr_vals)
...
num_chunks = 80  # reduce/increase depending on runtime budget
